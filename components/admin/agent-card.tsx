...
export function AgentCard({ agent, onDelete, variant = "default", isAdmin = false }: AgentCardProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (result.success) {
        ...
        toast({
          title: "Error",
          description: `Failed to delete agent: ${result.error || "Unknown error"}`,
          variant: "destructive",
        })
      }
    } catch (error) {
  ...
  return (
    <>
      <Card className="overflow-hidden h-full transition-all hover:shadow-md">
        ...
        <CardContent className="p-4 cursor-pointer" onClick={() => setDetailsOpen(true)}>
